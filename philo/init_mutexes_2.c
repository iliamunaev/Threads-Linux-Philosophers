...
/*   Updated: 2025/03/18 17:10:26 by imunaev-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/**
 * @file init_mutexes_2.c
 * @brief Mutex initialization functions for shared resources in the philosopher simulation.
 *
 ...
 *
 * This mutex ensures that only one thread at a time can write to standard output,
 * preventing interleaved log messages from different philosophers.
 *
 ...
 * If initialization fails, it cleans up by destroying the previously initialized
 * `print_mutex` to avoid resource leaks.
 *
 * @param env Pointer to the environment structure.
 * @return int Returns EXIT_SUCCESS on successful initialization, or
 * EXIT_FAILURE if mutex initialization fails.
